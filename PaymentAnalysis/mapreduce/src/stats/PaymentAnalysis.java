...
public class PaymentAnalysis extends Configured implements Tool
{
    /**
     ...
     * values are sent to one reducer.
     */
    public static class DatePartitioner implements
			Partitioner<TextDatePair, Text>
    {
	@Override
	public void configure(JobConf job)
	{
	...
	/* Define the job */
	JobConf conf = new JobConf(PaymentAnalysis.class);
	...
	conf = new JobConf(PaymentAnalysis.class);
    ...
    {
	System.out.println("Usage: PaymentAnalysisBetter <input path> <output path> [output mode(0 for sequence | 1 for text)] [Remove existing output (true | false)]");
	System.exit(0);
    }
	...
	int exitCode = ToolRunner.run(new PaymentAnalysis(), args);
